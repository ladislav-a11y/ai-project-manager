...
def test_json_schema_validation_block_is_recovered_and_dispatched_same_tick(tmp_path):
    project = ProjectRecord(
        name="Dashboard",
        priority=3,
        status=ProjectStatus.BLOCKED,
        orchestrator_ready_task="Implement the live status widget",
        checkpoint={"run_id": "schema-run", "completed_dod_indices": []},
        blocked_by=(
            "Error code: 400 - {'error': {'message': "
    ...
    subprocess_run, calls = _fake_ai_orchestrator(
        ...
        responses={
            "Dashboard": {
                "checkpoint": {"run_id": "schema-run-2", "completed_dod_indices": [0]},
                "last_output": "continued after structured-output fix",
                "next_step": "audit",
                "status": "in_progress",
            }
        },
